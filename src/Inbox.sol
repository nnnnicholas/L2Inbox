...
contract Inbox {
    ...
    event MessageRevealed(address indexed revealer, string plaintext);

    // Function to send a message
    function sendMessage(address recipient, bytes calldata encryptedMessage, bytes32 plaintextHash) external payable {
        // Forward any ether sent with the transaction to the recipient
        (bool success, ) = payable(recipient).call{value: msg.value}("");
        require(success, "Payment failed");

        // Emit the event
        emit MessageSent(msg.sender, recipient, plaintextHash, encryptedMessage, msg.value);
    }
